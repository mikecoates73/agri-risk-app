...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('"Area"', area)
      .eq('"Item"', item)
      .eq('"Element"', 'Gross Production Value (current thousand US$)')
      .not('"Value"', 'is', null)
      .order('"Year"', { ascending: true });

    if (error) {
      console.error('Error fetching FAOSTAT Production Value data:', error);
      return NextResponse.json(
    ...
    if (!data || data.length === 0) {
      return NextResponse.json(
        { error: 'No Production Value data found for the selected area and item' },
        { status: 404 }
      );
    }
